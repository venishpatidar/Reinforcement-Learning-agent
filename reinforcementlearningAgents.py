...
class ReinforcementLearingAgent(Agent):
    def __init__(
        self, filename=None, agent=None, numTraining=100, epsilon=0.05, alpha=2e-4, gamma=0.9
    ):
    ...
    def results(self):
        ...
        if self.episode % NUM_EPS_UPDATE == 0:
            ...
            if self.episode <= self.numTraining:
                ...
                print(
                    "\tCompleted %d out of %d training episodes" % (self.episode, self.numTraining)
                )
                ...
                if self.filename:
                    ...
                    with open(
                        "./results/" + self.agent + "/train_window_avg_" + self.filename, "a"
                    ) as f:
            ...
            # In testing
                ...
                if self.filename:
                    ...
                    with open(
                        "./results/" + self.agent + "/test_window_avg_" + self.filename, "a"
                    ) as f:
                        f.write(str(window_avg) + "\n")
    ...
    def optimize(self, lr, factor):
        ...
        for feature in self.weights:
            self.weights[feature] = self.weights[feature] + (
                lr * factor * self.grad_weights[feature]
            )
...
class ReinforceAgent(ReinforcementLearingAgent):
    ...
    def getAction(self, state):
        ...
        features_s_a = [
            self.feature_extractor.getFeatures(state, action) for action in legal_action
        ]
        ...
        if self.numTraining <= self.episode:
            ...
            action_index = random.choices(
                [
                    action_probs.index(max(action_probs)),
                    random.choices(range(len(action_probs)), weights=action_probs)[0],
                ],
